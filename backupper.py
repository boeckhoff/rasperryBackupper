import commands, time, traceback
...
def copyFromTo(device1, device2):
	...
	files = executeCommand('find ' + device1.path + ' -type f | wc -l').split('\n')
	...
	writeToLog('Saving file to ' + device2.curPath + ' on target device')

	for f in files:
		GPIO.output(OUT2_PIN,1)
		filePath = device1.path + f[2:]
		writeToLog('cp --parents ' + filePath + ' ' + device2.curPath)
...
class device:
	...
	def getPath(self):
		return self.line[self.line.index('%')+2:]
...
try:
	...
	while(1):	
		if device1 == None:
			new_output = executeCommand('df | grep media/pi')
		else:
			new_output = executeCommand('df | grep media/pi | grep -v ' + '"' + device1.path + '"')
